...
#        self.service_app.run()
    ...
    def test_storage_readings_direct(self):
        def comparator(*args):
#            print(args[0])
            return True

        assertResult = self.assert_post_request("sample_inputs_outputs/input_storage_direct.json", "sample_inputs_outputs/input_storage_direct.json", "http://localhost:5000/api/iot-contents", comparator)        
        self.assertTrue(assertResult)
        
    def test_storage_readings_redirect(self):
        def comparator(*args):
#            print(args[0])
            return True

        assertResult = self.assert_post_request("sample_inputs_outputs/input_storage_redirect.json", "sample_inputs_outputs/input_storage_redirect.json", "http://localhost:5000/api/iot-contents", comparator)        
        self.assertTrue(assertResult)
        
    def test_searcher_readings(self):
        def comparator(*args):
            print(args[0])
            return True

        assertResult = self.assert_post_request("sample_inputs_outputs/input_searcher.json", "sample_inputs_outputs/input_searcher.json", "http://localhost:5000/api/queries", comparator)        
        self.assertTrue(assertResult)
